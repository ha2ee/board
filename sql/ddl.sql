CREATE TABLE `tb_post` (
    ...
    `created_date`  datetime      NOT NULL DEFAULT current_timestamp() COMMENT '최초 등록일',
    `modified_date` datetime               DEFAULT NULL COMMENT '최종 수정일',
    PRIMARY KEY (`id`)
) COMMENT '게시글';
...
create table tb_comment (
    ...
    , delete_yn tinyint(1) not null comment '삭제 여부'
    , created_date datetime not null default CURRENT_TIMESTAMP comment '최초 등록일'
    , modified_date datetime comment '최종 수정일'
    , primary key(id)
) comment '댓글';
